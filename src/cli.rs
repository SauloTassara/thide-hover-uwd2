...
const WM_THIDE_PATCH_WATERMARK: u32 = WM_APP + 5;

const IPC_WINDOW_CLASS: &str = "THideHoverUwd2IPCWindow";
...
pub fn handle_cli_command(args: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args[0].to_lowercase().as_str() {
        ...
        "patch-watermark" | "repatch-watermark" => send_command(
            WM_THIDE_PATCH_WATERMARK,
            "Requested Insider watermark re-patch...",
        ),
...
pub const fn get_patch_message_id() -> u32 {
    WM_THIDE_PATCH_WATERMARK
}
...
fn enable_autostart() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let key_path: Vec<u16> = "Software\\Microsoft\\Windows\\CurrentVersion\\Run\0"
        .encode_utf16()
        .collect();
    let value_name: Vec<u16> = "THideHoverUwd2\0".encode_utf16().collect();
...
fn print_usage() {
    println!("THide Hover + UWD2 - Taskbar Hide Utility");
    println!();
    println!("USAGE:");
    ...
    println!("    patch-watermark    Re-apply the Insider watermark patch");
